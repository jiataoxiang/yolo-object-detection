...
from YoloV2mod import YoloV2mod
from util import (
    nonMaxSuppression,
...
NUM_WORKERS = 1
PIN_MEMORY = True
SAVE_MODEL_FILE = "pretrainedYolo.pth.tar"
...
def main():
    model = YoloV2mod(num_boxes=2, num_classes=2).to(DEVICE)
    ...
    plt.xlabel("Epoch")
    plt.ylabel("mAP")
    plt.savefig("trainV2mod_mAP.jpg")

    plt.plot(np.arange(epoch + 1), np.array(losses), "b")
    plt.xlabel("Epoch")
    plt.ylabel("mean loss")
    plt.savefig("trainV2mod_Loss.jpg")
