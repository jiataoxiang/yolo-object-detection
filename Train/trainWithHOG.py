...
NUM_WORKERS = 1
PIN_MEMORY = True
SAVE_MODEL_FILE = "YoloWithHOG.pth.tar"
...
def main():
    ...
    model = pretrainedYoloWithHOG(model=vgg16, split_size=7, num_boxes=2, num_classes=2).to(DEVICE)
    ...
    plt.xlabel("Epoch")
    plt.ylabel("mAP")
    plt.savefig("trainWithHOGmAP1.jpg")

    plt.plot(np.arange(epoch + 1), np.array(losses), "b")
    plt.xlabel("Epoch")
    plt.ylabel("mean loss")
    plt.savefig("trainWithHOGLoss1.jpg")
